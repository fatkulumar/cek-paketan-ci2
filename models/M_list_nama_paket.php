<?php
    ...
    class M_list_nama_paket extends CI_Model{
        ...
        function get_datatables()
        {
            ...
            // $query = $this->db->get();
            $query = $this->db
            ->select('`id_paket`, `id_santri`, `nama_paket`, `hp`, count(*) as jumlah')
        ...
        function tampil_data_asc(){
            return $this->db
            ->select('`id_paket`, `id_santri`, `nama_paket`, `penerima`, `jenis_kirim`, `tgl_terima`, `tgl_ambil`, `pengambil`, `status_ambil`, `hp`, `creat_at`, `modified_at`, `tahun`, `jml_paket`, count(*) as jumlah')
            ->order_by('id_paket','desc')
            // ->where('status_ambil IS', 'NULL', false)
            ->get($this->table)->result_array();
        }

        function jumlah_list_nama_paket(){
            $jumlah = $this->db
            ->select('count(id_paket) as jumlah')
            ->order_by('id_paket','desc')
            ->group_by('nama_paket')
            // ->where('status_ambil IS', 'NULL', false)
            ->get($this->table)->result_array();
        ...
        function total_list_nama($tahun) {
            return $this->db
            ->select('nama_paket')
            ->where('tahun', $tahun)
            ->get($this->table)->result_array();
